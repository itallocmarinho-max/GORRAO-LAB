  ...
  return (
    <div ref={menuRef} className="relative">
      <button
        ...
        className="flex h-9 w-11 items-center justify-center border-0 bg-transparent text-[#39FF14] transition-all hover:bg-transparent"
      >
        <span className="relative block h-5 w-6" aria-hidden>
          <span
            className={`absolute left-1/2 top-1/2 h-[2px] w-4 -translate-x-1/2 bg-current shadow-[0_0_5px_currentColor] transition-all duration-300 ${aberto ? "-translate-y-1/2 rotate-45" : "-translate-y-[5px] rotate-0"}`}
          />
          <span
            className={`absolute left-1/2 top-1/2 h-[2px] w-4 -translate-x-1/2 bg-current shadow-[0_0_5px_currentColor] transition-all duration-300 ${aberto ? "-translate-y-1/2 -rotate-45" : "translate-y-[3px] rotate-0"}`}
          />
        </span>
      </button>
      ...
      >
        <nav className="space-y-1 border-b border-white/10 pb-3">
          ...
          {(isDiretor || isAdmin) && (
            <Link
              to="/financeiro"
              onClick={() => setAberto(false)}
              className={`block border px-3 py-2 text-[10px] font-bold uppercase tracking-[0.16em] transition ${pathname === "/financeiro" ? "border-[#39FF14]/55 bg-[#39FF14]/10 text-[#39FF14]" : "border-transparent text-white/70 hover:border-[#39FF14]/25 hover:bg-white/[0.04] hover:text-[#39FF14]"}`}
            >
              / / FINANCEIRO
            </Link>
          )}
        ...
        <div className="mt-3 border border-white/10 bg-white/[0.035] p-3">
          <div className="truncate text-[10px] font-bold uppercase tracking-[0.15em] text-[#39FF14]">
            {nome || user?.email}
          </div>
          ...
          <div className="mt-3 flex gap-2">
            {role === "admin" && (
              <Link
                to="/admin/painel"
                onClick={() => setAberto(false)}
                className="flex h-9 flex-1 items-center justify-center gap-2 border border-[#39FF14]/30 text-[9px] font-bold uppercase tracking-wider text-[#39FF14] transition hover:bg-[#39FF14]/10"
              >
            ...
            <button
              type="button"
              onClick={sair}
              className="flex h-9 flex-1 items-center justify-center gap-2 border border-white/15 text-[9px] font-bold uppercase tracking-wider text-white/60 transition hover:border-red-500/40 hover:bg-red-500/10 hover:text-red-400"
            >
