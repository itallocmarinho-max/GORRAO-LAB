...
import { ArrowLeft, Link2, RefreshCw, Search, Sheet } from "lucide-react";
...
function AdminVendas() {
    ...
    try {
      ...
      toast.success(
        `${result.synchronized} venda(s) sincronizada(s)` +
          `; ${result.termosReserva} termo(s) reserva sincronizado(s)` +
          (result.removed ? `; ${result.removed} removida(s)` : "") +
          (result.termosReservaRemoved
            ? `; ${result.termosReservaRemoved} termo(s) removido(s)`
            : "") +
  ...
  const totalGorrao = rows.reduce((total, row) => total + Number(row.credito_gorrao || 0), 0);
  const filteredGorrao = filtered.reduce(
    (total, row) => total + Number(row.credito_gorrao || 0),
    0,
  );
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <Card>
              <CardHeader className="pb-2">
                <CardDescription>Vendas da Diretoria Gorrão</CardDescription>
                <CardTitle>
                  {totalGorrao.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}
                </CardTitle>
              </CardHeader>
            </Card>
            <Card>
              <CardHeader className="pb-2">
                ...
                <CardTitle>
                  {filteredGorrao.toLocaleString("pt-BR", { maximumFractionDigits: 1 })}
                </CardTitle>
              </CardHeader>
            </Card>
            <Card>
              ...
              <table className="w-full min-w-[1700px] text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-left text-xs uppercase tracking-wide text-white/50">
                    ...
                    <th className="px-3 py-3">Diretor principal</th>
                    <th className="px-3 py-3">Sup principal</th>
                    <th className="px-3 py-3">Gerente principal</th>
                    <th className="px-3 py-3">Corretor principal</th>
                    <th className="px-3 py-3">Diretor fifty</th>
                    <th className="px-3 py-3">Sup fifty</th>
                    <th className="px-3 py-3">Gerente fifty</th>
                    <th className="px-3 py-3">Corretor fifty</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={13} className="px-3 py-10 text-center text-white/50">
                        Carregando…
                      </td>
                    </tr>
                  ) : filtered.length === 0 ? (
                    ...
                    filtered.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-3 py-3">{row.diretor_fifty || "—"}</td>
                        <td className="px-3 py-3">{row.superintendente_fifty || "—"}</td>
                        <td className="px-3 py-3">{row.gerente_fifty || "—"}</td>
                        <td className="px-3 py-3">{row.corretor_fifty || "—"}</td>
                      </tr>
                    ))
                  )}
                </tbody>
